<template>
    ...
    <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
				...
				<input v-model="searchForm.rizhibiaoti" type="text" placeholder="日志标题" ></input>
			</view>
        ...
        <view v-if="index%6==0" @tap="onDetailTap(product)" class="list-item" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0 0 40rpx 0","borderColor":"red","backgroundColor":"rgba(6, 6, 6, 0.27)","borderRadius":"20rpx","borderWidth":"0","width":"100%","borderStyle":"solid","height":"320rpx"}'>
            ...
            <view :style='{"padding":"24rpx","boxShadow":"0 0 0px rgba(255,0,0,0)","margin":"-80rpx 0 0 0","borderColor":"red","backgroundColor":"rgba(0, 0, 0, 0.45)","color":"rgba(255, 255, 255, 1)","textAlign":"left","borderRadius":"0 0 20rpx 20rpx","borderWidth":"0","width":"100%","lineHeight":"36rpx","fontSize":"28rpx","borderStyle":"solid"}' class="title">{{product.rizhibiaoti}}</view>
                                                                                                                                                                                                                                                                      </view>
                                                                                                                                                                                                                  <view style="display: flex;justify-content: space-between;">
            <text v-if="userid && isAuth('chongwurizhi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="!userid && isAuthFront('chongwurizhi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="userid && isAuth('chongwurizhi','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
            <text v-if="!userid && isAuthFront('chongwurizhi','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
          </view>
        </view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.rizhibiaoti){
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`chongwurizhi`, params);
                } else {
                    res = await this.$api.list(`chongwurizhi`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('chongwurizhi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.rizhibiaoti){
					searchForm['rizhibiaoti'] = '%' + this.searchForm.rizhibiaoti + '%'
				}
				if(this.searchForm.xingming){
                ...
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`chongwurizhi`, searchForm);
                } else {
                    res = await this.$api.list(`chongwurizhi`, searchForm);
                }
				// 如果是第一页数据置空
